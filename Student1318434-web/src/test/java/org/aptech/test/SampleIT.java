...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "Student1318434-ear.ear");

        // Current directory is the root of the "Student1318434-web" project. Go up one level
        // and enter the "Student1318434-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../Student1318434-ejb/target/Student1318434-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "Student1318434-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/Student1318434-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "Student1318434-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
